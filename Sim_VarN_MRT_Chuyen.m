...
function[RSsum,RSsum_aprx,RS1,RS2,RS1_aprx,RS2_aprx,R1,...
        ...
        = Sim_VarN_MRT_Chuyen(QdB,PdB,Q0dB,beta,betaE,tau,T,MM,NN,iter)
...
%             z(:,m) = temp(:,1);
    ...
    if(m>1)    
        for j=1:m-1
            I114(n,m) = I114(n,m) + beta(m,1)*((Q(j,1)+Q(j,2))+Q0(j));
            I124(n,m) = I124(n,m) + beta(m,2)*((Q(j,1)+Q(j,2))+Q0(j));
    ...
    R12_aprx(n,m) = (1-tau/T)*log2(1+Q(m,1)*beta(m,2)*rho(m,2)*NN(n)/...
        (I111(n,m)+I112(n,m)+I113(n,m)+I114(n,m)+1));%due to EU2 decode EU1         
    R1_aprx(n,m) = min(R12_aprx(n,m),R11_aprx(n,m));
    
    %rate at EU2---------------------------------------
    R2(n,m) = (1-tau/T)*log2(1+Theta2(n,m)/...
        (I21(n,m)+I22_sim(n,m)+I23_sim(n,m)+1));        
    R2_aprx(n,m) = (1-tau/T)*log2(1+Q(m,2)*beta(m,2)*rho(m,2)*NN(n)/...
