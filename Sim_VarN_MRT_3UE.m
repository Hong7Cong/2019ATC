...
function[RSsum,RSsum_aprx,RS1,RS2,RS3,RS1_aprx,RS2_aprx,RS3_aprx,R1,...
        ...
        = Sim_VarN_MRT_3UE(QdB,PdB,Q0dB,beta,betaE,tau,T,MM,NN,iter)
...
%             W(:,m) = V(:,m)/norm(V(:,m));
    ...
    I32(n,m) = 0;%second term of (16), AN leakage
    ...
    if(m>1)    
        for j=1:m-1
            I114(n,m) = I114(n,m) + beta(m,1)*((Q(j,1)+Q(j,2)+Q(j,3))+Q0(j));%ADD Q(j,3)
            I124(n,m) = I124(n,m) + beta(m,2)*((Q(j,1)+Q(j,2)+Q(j,3))+Q0(j));%ADD Q(j,3)
            I134(n,m) = I134(n,m) + beta(m,3)*((Q(j,1)+Q(j,2)+Q(j,3))+Q0(j));%ADD
    ...
    R11_aprx(n,m) = (1-tau/T)*log2(1+Q(m,1)*beta(m,1)*rho(m,1)*(NN(n))/...
        (I111(n,m)+I112(n,m)+I113(n,m)+I114(n,m)+1));%due to EU1 decode itself
    R12_aprx(n,m) = (1-tau/T)*log2(1+Q(m,1)*beta(m,2)*rho(m,2)*(NN(n))/...
        (I121(n,m)+I122(n,m)+I123(n,m)+I124(n,m)+1));%due to EU2 decode EU1      
    R13_aprx(n,m) = (1-tau/T)*log2(1+Q(m,1)*beta(m,3)*rho(m,3)*(NN(n))/...
        (I131(n,m)+I132(n,m)+I133(n,m)+I134(n,m)+1));%due to EU3 decode EU1    
    ...
    R22_aprx(n,m) = (1-tau/T)*log2(1+Q(m,2)*beta(m,2)*rho(m,2)*(NN(n))/...
        (I21(n,m)+I22(n,m)+I23(n,m)+1));
    R23(n,m) = (1-tau/T)*log2(1+Theta23(n,m)/...%due to EU3 decode UE2
        (I231(n,m)+I232(n,m)+I233(n,m)+1));     
    R23_aprx(n,m) = (1-tau/T)*log2(1+Q(m,3)*beta(m,3)*rho(m,3)*(NN(n))/...
        (I231(n,m)+I232(n,m)+I233(n,m)+1));
    ...
    R3_aprx(n,m) = (1-tau/T)*log2(1+Q(m,3)*beta(m,3)*rho(m,3)*(NN(n))/...
